...
class Field(width: Int, height: Int) extends JComponent:
  ...
  private def ballImpact(ball: Ball, actor: Actor, time: Double): Option[Impact] =
    val path = LineSegment(ball.location, ball.location + ball.velocity * time)

    actor match
      case Wall(segment) =>
        path.intersection(segment).map { point =>
          val impactTime = (point - ball.location).length / ball.velocity.length
          Impact(actor, point, ball.velocity, impactTime)
        }
      case Bumper(circle) =>
        path.intersection(circle).map { point =>
          val impactTime = (point - ball.location).length / ball.velocity.length
          Impact(actor, point, ball.velocity, impactTime)
        }
      case _ => None
  ...
  private def findBallImpacts(ball: Ball, time: Double): Seq[Impact] =
    actors.flatMap(actor => ballImpact(ball, actor, time))
  ...
  private def findNextBallImpact(ball: Ball, time: Double, lastBounced: Option[Actor] = None): Option[Impact] =
    findBallImpacts(ball, time)
      .filter(impact => lastBounced.contains(impact))
      .sortBy(_.time)
      .headOption
   ...
   */
  final def updateActor(actor: Actor, time: Double, lastBounced: Option[Actor] = None): Actor = actor match
    case Ball(location, velocity) =>
      findNextBallImpact(Ball(location, velocity), time, lastBounced) match
        case None =>
          //Ball((location + (velocity * time)), velocity)
          val newLocation = Ball(location, velocity).location + (Ball(location, velocity).velocity * time)
          Ball(location, velocity).copy(location = newLocation)
        case Some(impact) =>
          val newBall = Ball(location, velocity).copy(location = impact.point, velocity = Ball(location, velocity).velocity.reflect(impact.normal))
          updateActor(newBall, time - impact.time, Some(impact.impactedActor))

          /*val newBall = Ball(impact.point, velocity.reflect(impact.normal))
          updateActor(newBall, time - impact.time, Some(actor))*/
    case _ => actor
